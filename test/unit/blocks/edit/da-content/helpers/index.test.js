...
describe('UE URLs', () => {
  it('Supports single sheet configs', async () => {
    ...
    try {
      ...
      const ueUrl = await ueUrlHelper('aabsites', 'gov', 'https://main--gov--geometrixx.ent-aem.page/query-builder');
      expect(ueUrl).to.equal('https://experience.adobe.com/#/@sitesinternal/aem/editor/canvas/main--gov--geometrixx.ue.ent-da.page/query-builder');
    } finally {
  ...
  it('Successfully dies gracefully', async () => {
    ...
    try {
      window.fetch = mockFetch;
      const ueUrl = await ueUrlHelper('aabsites-gone', 'gov', 'https://main--gov--geometrixx.ent-aem.page/query-builder');
  ...
  it('Returns null when no editor.path or quick-edit config exists', async () => {
    ...
    try {
      ...
      const url = await ueUrlHelper('org', 'repo', 'https://main--repo--org.ent-aem.page/page');
  ...
  it('Builds a quick-edit URL when quick-edit config matches the repo', async () => {
    ...
    try {
      ...
      const url = await ueUrlHelper('org-qe', 'repo', 'https://main--repo--org.ent-aem.live/page');
      expect(url).to.equal('https://main--repo--org.ent-aem.page/page?quick-edit=on');
    } finally {
      window.fetch = orgFetch;
    }
  });
  ...
  it('Strips trailing /index when building the quick-edit URL', async () => {
    ...
    try {
      ...
      const url = await ueUrlHelper('org-qe-strip', 'repo', 'https://main--repo--org.ent-aem.live/folder/index');
      expect(url).to.equal('https://main--repo--org.ent-aem.page/folder/?quick-edit=on');
    } finally {
      window.fetch = orgFetch;
    }
  });
  ...
  it('getUeUrl returns null when ueConf has no value', async () => {
    ...
    const result = await getUeUrl({}, 'https://main--repo--org.ent-aem.page/page');
    expect(result).to.equal(null);
  });

  it('getUeUrl returns null when no @org appears in the editor.path value', async () => {
    const { getUeUrl } = await import('../../../../../../blocks/edit/da-content/helpers/index.js');
    const result = await getUeUrl({ value: '/no-at-org' }, 'https://main--repo--org.ent-aem.page/page');
    expect(result).to.equal(null);
  });
});
