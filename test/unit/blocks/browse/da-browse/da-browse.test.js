...
describe('Browse', () => {
  it('Pasted item uses the target URL', async () => {
    ...
    try {
      ...
      expect(copyCall.url).to.equal('https://admin.ent-da.live/copy/myorg/mysite/myroot/srcdir/d1.html');
  ...
  it('Load more uses da-continuation-token request header', async () => {
    ...
    try {
      ...
      expect(fetchedArgs[0].url).to.equal('https://admin.ent-da.live/list/myorg/mysite/myroot/destdir');
...
describe('DaBrowse Component', () => {
  ...
  describe('getEditor', () => {
    ...
    it('matches a single editor.path row by prefix', async () => {
      ...
      mockConfig([{ key: 'editor.path', value: '/myorg-a/mysite=https://experience.adobe.com/#/@dxorg/aem/editor/canvas/main--mysite--myorg.ue.ent-da.live' }]);
      const url = await daBrowseComp.getEditor(true);
      expect(url).to.equal('https://experience.adobe.com/#/@dxorg/aem/editor/canvas/main--mysite--myorg.ue.ent-da.live');
    });
    ...
    it('prefers the more specific (longer prefix) match over a shorter one', async () => {
      ...
      mockConfig([
        { key: 'editor.path', value: '/myorg-b/mysite=https://experience.adobe.com/#/@dxorg/aem/editor/canvas/main--mysite--myorg.ue.ent-da.live' },
        { key: 'editor.path', value: '/myorg-b/mysite/dealers=https://ent-da.live/form#' },
      ]);
      const url = await daBrowseComp.getEditor(true);
      // /myorg-b/mysite/dealers is more specific than /myorg-b/mysite even though
      // UE_CONF has a longer total string length
      expect(url).to.equal('https://ent-da.live/form#');
    });
    ...
    it('falls back to the broader match when path is outside the specific folder', async () => {
      ...
      mockConfig([
        { key: 'editor.path', value: '/myorg-c/mysite=https://experience.adobe.com/#/@dxorg/aem/editor/canvas/main--mysite--myorg.ue.ent-da.live' },
        { key: 'editor.path', value: '/myorg-c/mysite/dealers=https://ent-da.live/form#' },
      ]);
      const url = await daBrowseComp.getEditor(true);
      expect(url).to.equal('https://experience.adobe.com/#/@dxorg/aem/editor/canvas/main--mysite--myorg.ue.ent-da.live');
    });
    ...
    it('returns default edit path when no prefix matches the current path', async () => {
      ...
      mockConfig([{ key: 'editor.path', value: '/myorg/mysite=https://experience.adobe.com/#/@dxorg/aem/editor/canvas/main--mysite--myorg.ue.ent-da.live' }]);
      const url = await daBrowseComp.getEditor(true);
      expect(url).to.equal('/edit#');
    });
  });
