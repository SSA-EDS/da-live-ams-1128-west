...
describe('DaSites', () => {
  ...
  describe('parseSubdomain', () => {
    it('Extracts org and repo from a ent-aem.live URL', () => {
      const el = new DaSites();
      const result = el.parseSubdomain('https://main--repo--org.ent-aem.live/');
      expect(result).to.equal('#/org/repo');
    });

    it('Extracts org and repo from an aem.page URL', () => {
      const el = new DaSites();
      const result = el.parseSubdomain('https://main--my-site--my-org.ent-aem.page/');
    ...
    it('Returns null for a malformed helix-style hostname missing org', () => {
      const el = new DaSites();
      expect(el.parseSubdomain('https://main--repo.ent-aem.live/')).to.equal(null);
    });
